...
import { Activity, User, Mail, Lock, ArrowRight, AlertCircle } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      storeLogin(loginData.token, loginData.user)
      navigate('/dashboard')
    } catch (err) {
  ...
  const inputClass = "w-full bg-slate-950 border border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all"
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-400 mt-1">Start receiving real-time notifications</p>
        </div>
